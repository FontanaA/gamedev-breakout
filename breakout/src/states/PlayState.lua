...
function PlayState:update(dt)
    ...
    if self.score > 0 and ispowerup < 1 and not (ispowerup == 489) then
        --Powerup.init()
        ispowerup = 100
    end
    if b2 == 2 then
    ...
    if ispowerup > 1 then
        ...
        if powerup.y < VIRTUAL_HEIGHT then
            ...
            if powerup.y > self.paddle.y then
                --print("y")
                if powerup.x > self.paddle.x then
                    if powerup.x < self.paddle.x + self.paddle.width then
    ...
    for k, brick in pairs(self.bricks) do
        ...
        if brick.inPlay and self.ball:collides(brick) or f2 == 1 or f3 == 1 then

            -- add to score
            self.score = self.score + (brick.tier * 200 + brick.color * 25)
            --if self.score % 200 == 0 and self.Paddle.size < 4 then --increase size based on
            --local score = self.score
            if self.score >= 200 and self.score < 800 then
                self.paddle.size = 3
            end
